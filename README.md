...
These improved results were in part due to adding the predictions generated from Approach II (outlined above) as another feature to our Approach I model. 
...
With Approach II, we managed to build a model which was able to catch the seasonality of the fires but considered the effect of the past 30 days weather. With this approach, the model didn’t know the existing fires, so it shows the cells with the best conditions for possible fires based on the prior 30 days data. To retrieve the information from these predictions the earlier described thresholding method was necessary because almost all of the predicted values were below zero. We had to find the best separator of non-fiery and fiery cells and based on that threshold we got the maps below.
