...
def _convert_attachment_data_to_dataframe(data, data_type, cols=None):
    str_decoded  = base64.urlsafe_b64decode(data.encode('UTF-8'))
    if data_type == 'csv':
        df = pd.read_csv(StringIO(str_decoded), usecols=cols)
    elif data_type == 'xlsx':
...
def get_csv_or_xl_attachments_from_msg_id(service, messageId, cols=None):
    ...
    dfs = [_convert_attachment_data_to_dataframe(d, t, cols=cols)
            for d, t in zip(datas, types)]
    return [{'emailsubject': subject, 'filename': f, 'data': d}
            for f, d in zip(filenames, dfs)]


def query_for_csv_or_xl_attachments(service, search_query, cols=None):
    message_ids = query_for_message_ids(service, search_query)
    csvs = []
    for msg_id in message_ids:
        loop_csvs = get_csv_or_xl_attachments_from_msg_id(service, msg_id, cols=cols)
